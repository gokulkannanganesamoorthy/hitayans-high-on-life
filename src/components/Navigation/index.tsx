...
export default function Navigation() {
  ...
  return (
    <>
      ...
      <header
        className={`main-header ${isScrolled ? 'scrolled' : ''} ${isOpen ? 'menu-open' : ''}`}
      >
        <div className="header-inner">
          <div className="header-left">
            <Link
              ...
              data-cursor-view="PASS"
            >
              RESERVE PASS
            </Link>
            <button
              className={`nav-sound-btn ${isPlaying ? 'playing' : ''}`}
              onClick={toggleSound}
              aria-label="Toggle Sound"
              data-cursor-view="SOUND"
            >
              {isPlaying ? (
                <svg
                  viewBox="0 0 24 24"
          ...
          <Link
            to="/"
            className="logo-link center-logo"
            data-cursor-view="HOL"
            aria-label="Home"
          >
            <img
              src="/assets/logo.png"
          ...
          <div className="header-right">
            <button
              className="menu-toggle nav-text-link"
              onClick={toggleMenu}
              aria-label="Toggle Menu"
              data-cursor-view="MENU"
            >
              <span className="menu-text hamburger-icon">
                {isOpen ? 'CLOSE' : '≡'}
              </span>
            </button>
          </div>
